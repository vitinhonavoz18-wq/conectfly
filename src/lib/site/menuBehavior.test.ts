...
import { MENU_LAYOUTS, type LayoutId } from "./menuLayout";
...
describe("os padrões de cada layout de segmento", () => {
  it("todo padrão declarado é um modo que existe", () => {
    for (const id of Object.keys(MENU_LAYOUTS) as LayoutId[]) {
      const padrao = MENU_LAYOUTS[id].modoDeNavegacaoPadrao;
  ...
  it("em TODO layout, a escolha do lojista vale nos três modos", () => {
    ...
    for (const id of Object.keys(MENU_LAYOUTS) as LayoutId[]) {
      const padrao = MENU_LAYOUTS[id].modoDeNavegacaoPadrao ?? null;
      for (const modo of MODOS_DE_NAVEGACAO) {
