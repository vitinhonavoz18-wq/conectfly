...
import type { ZcRegionRow, ZcTariffRow, ZcVehicleCategoryRow } from "../db/types";
...
export async function getPublicCatalog() {
  const [regions, categories] = await Promise.all([listRegions(), listVehicleCategories()]);
  ...
  return {
    ...
    tariffs: tariffs
      ...
      .map((tariff) => ({
        ...
        immediate_surcharge_pct: tariff.immediate_surcharge_pct,
      })),
  };
}
