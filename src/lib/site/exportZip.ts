...
export function MenuSection() {
  const [act, setAct] = useState(menuCategories[0]?.id ?? "");
  if (menuCategories.length === 0) return null;
  const cur = menuCategories.find((c) => c.id === act) ?? menuCategories[0];
  return (
    <section id="cardapio" className="py-16 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-site-fg/70 mt-2">Escolha seus pratos favoritos</p>
        </div>
